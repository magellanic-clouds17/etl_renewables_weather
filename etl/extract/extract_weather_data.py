...
# Download and extract function for weather data
def download_and_extract_file(url, save_path, extract_to):
...
## download metadata file function

def download_metadata_and_convert_to_csv(url_meta, save_path_meta, save_path_csv):
    response = requests.get(url_meta)
    if response.status_code == 200:
        # Ensure the directory exists where the .txt file will be saved
        os.makedirs(os.path.dirname(save_path_meta), exist_ok=True)
        
        # Save the .txt file
        with open(save_path_meta, 'wb') as file:
            file.write(response.content)
        print("Metadata file downloaded successfully")
        
        # Read the fixed-width formatted .txt file
        # Note: You might need to manually specify the column widths if auto-detection isn't accurate
        df = pd.read_fwf(save_path_meta, encoding='latin1')
        
        # Save the DataFrame as a .csv file
        df.to_csv(save_path_csv, index=False, sep=';')
        print(f"Metadata converted and saved as {save_path_csv}")
...
## transform the metadata file to df
...
# use download_and_extract_file function to download and extract all the air temperature data files
